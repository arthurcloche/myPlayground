...
unY = indy.filter( onlyUnique );
unX = indx.filter( onlyUnique );

for (let i = 0; i < unX.length; i++) {
  minX = min(unX);
  maxX = max(unX);
}

for (let i = 0; i < unY.length; i++) {
  minY = min(unY);
  maxY = max(unY);
}
...
function createIdI(x){

  return floor(map(x,minX,maxX,0,maxX*5));
}

function createIdJ(y){

  return floor(map(y,minY,maxY,0,maxY*8));

}

function displacerX(i,j,x){
  ...
  //   f: dfx.value(),
  // }
  this.subrad = dbx.value();
  return this.subrad * sin(time+((i)/TWO_PI)+((j)/TWO_PI));;


}

function displacerY(i,j,y){
...
this.subrady = dby.value();
return this.subrady * cos(time+((i)/TWO_PI)+((j)/TWO_PI));
...
function createL(t, x, y, s) {
  ...
  if (this.break == 0 && this.breakb == 0) {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      let idi = createIdI(lt.x);
      let idj = createIdJ(lt.y);
      ...
      vertex(
        lt.x * dax.value()
        + displacerX(idi,idj,xx),

        lt.y * dafy.value()
        + displacerY(idi,idj,yy),
